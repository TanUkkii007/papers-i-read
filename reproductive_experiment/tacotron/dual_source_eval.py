...
def eval():
    ...
    with g.graph.as_default():
        ...
        with sv.managed_session(config=tf.ConfigProto(
            ...
            attention_final_state = sess.run(
                g.attention_final_state, {g.x1: X1,
                                          g.x2: X2,
                                          g.y: outputs1})
    ...
    with codecs.open(hp.outputdir + '/text.txt', 'w', 'utf-8') as fout:
        for i, (x1, x2, s, a1, a2) in enumerate(
                zip(X1, X2, outputs2,
                    attention_final_state.state1_alignment_history,
